...
typedef struct {
	...
	GLint			loc[RC_N_TRANSFORMATION_MATRICES + 2];
	...
	)
{
	...
	static GlVersion latestVersion = VER_GL2;
	static char *filename_prefix[] = {"gl4/", "gl3/", "gl2/"};
	static i32 n_shader_types[] = {5, 3, 2};

	static GlVersion shader_version[] = {VER_GL2, VER_GL2, VER_GL3, VER_GL4, VER_GL4};
	static ShaderType_t shader_type[] = {SHADER_TYPE_VERTEX, SHADER_TYPE_FRAGMENT, SHADER_TYPE_GEOMETRY, SHADER_TYPE_CONTROL, SHADER_TYPE_EVALUATION};
	static char *filename_suffix[] = {".vs", ".fs", ".gs", ".cs", ".es"};
	static char *shader_string[] = {"Vertex", "Fragment", "Geometry", "Control", "Evaluation"};
...
void Renderer::init()
{
#if !defined(RC_MACOSX)
	GLenum err = glewInit();

	if (err != GLEW_OK)
		REPORT_ERROR("Couldn't initiate glew: %s", glewGetErrorString(err));
#endif
...

	{
		...
		if (glver[0] == '4')
			latestVersion = VER_GL4;
		else if (glver[0] == '3')
			latestVersion = VER_GL3;
		else
			latestVersion = VER_GL2;
...
#endif /* RC_OPENGL_ES_20 / RC_OPENGL */
#endif
...
//		glGetIntegerv(GL_MAX_ELEMENTS_VERTICES, (GLint *) &limits.max_vertices);
		...
		glGetIntegerv(GL_MAX_TEXTURE_UNITS, (GLint *) &limits.max_texture_units);
	...
	)
{
	...
	if (shaderProgram)
		...
		sp = active_shader_program;

	RenderState *rs;
	if (state)
		rs = state;
	else if (shaderProgram && shaderProgram->getRenderState())
		rs = shaderProgram->getRenderState();
	else
		rs = sp->getRenderState();

	mat4f world = worldMatrix ? *worldMatrix : mat4f::identityMatrix();
	mat4f worldInverse = worldMatrixInverse ? *worldMatrixInverse : world.getInverse();
	...
	handle = (GLShaderProgramHandle_t *) sp->getHandle();
	...
	for (i = 0; i < RC_N_TRANSFORMATION_MATRICES; i++) {
		//GLint loc = glGetUniformLocation(programId, gl_matrix_transform[i]);
		...
		if (loc != -1) {
			...
			switch (which_matrix) {
				...
				case MAT_VIEW_PROJ:
					...
					break;
				case MAT_WORLD:
					mat = inverse ? worldInverse : world;
					break;
				case MAT_WORLD_VIEW:
					mat = inverse ? worldInverse * viewInverse : view * world;
					break;
				case MAT_WORLD_VIEW_PROJ:
					mat = inverse ? worldInverse * viewProjectionInverse : viewProjection * world;
					break;
			}

			/* IMPORTANT NOTE: GL wants transposed matrices, and the the transpose-parameter doesn't seem to work! */
			if (!transpose)
				mat.transpose();
	...
	/* Normal matrix */
	{
		GLint loc = handle->loc[RC_N_TRANSFORMATION_MATRICES + 1];
		if (loc != -1) {
			mat3f nm = worldInverse.rotationMatrix();
			glUniformMatrix3fv(loc, 1, GL_FALSE, nm.vec);
		}
	}
	GL_CHECK_ERRORS();
...
{
	mat4f eye = mat4f::identityMatrix();
	render(va, &eye, &eye, 0, shaderProgram, state);
}
	...
	)
{
	...
	if (shaderProgram)
		sp = shaderProgram;
	else if (mesh.getShaderProgram())
		sp = mesh.getShaderProgram();
	else
		sp = active_shader_program;
	if (!sp) {
		REPORT_ONCE("No shader program bound to geometry %s", mesh.getName());
		return;
	}
	...
	if (rs->flags & STATE_FRUSTUM_CULL) {
		AABox bbox;
	...
	render(*mesh.getVertexArray(), &worldMatrix, &worldMatrixInverse, &mesh, sp, rs);
}
...
{
	for (i32 i = 0; i < n; i++)
		render(*meshList[i], shaderProgram, state);
}
	...
	)
{
	...
	switch (type) {
		...
		case SHADER_TYPE_FRAGMENT:
			...
			GL_CHECK_ERRORS();
			break;
#if defined RC_OPENGL && !defined(RC_MACOSX)
		case SHADER_TYPE_GEOMETRY:
			id = glCreateShaderObjectARB(GL_GEOMETRY_SHADER);
			GL_CHECK_ERRORS();
			break;
		case SHADER_TYPE_CONTROL:
			id = glCreateShaderObjectARB(GL_TESS_CONTROL_SHADER);
			GL_CHECK_ERRORS();
			break;
		case SHADER_TYPE_EVALUATION:
			id = glCreateShaderObjectARB(GL_TESS_EVALUATION_SHADER);
			GL_CHECK_ERRORS();
			break;
#endif
		default:
	...
	if (status == GL_FALSE) {
        ...
        REPORT_WARNING("Couldn't compile %s shader for \"%s\"", shader_string[(i32) type], parent);
        return 0;
	} else {
		GLsizei length;
		char result[RC_RENDERER_ERR_BUF_LEN];
		glGetShaderInfoLog(id, RC_RENDERER_ERR_BUF_LEN, &length, result);
		if (length) {
			REPORT("%s shader compiled with a message (for \"%s\")", shader_string[(i32) type], parent);
			REPORT("%s\n --- ", result);
		}
	}
	...
	)
{
	...
	bool shaderFound[5] = {false, false, false, false, false};
	...
	for (i = 0; i < n; i++) {
		...
		const char *ext = strrchr(desc[i], '.');

		i32 s = 0;
		for (; s < n_shader_types[(i32) latestVersion]; s++) {
			if (strcmp(filename_suffix[s], ext) == 0)
				break;
		}
		if (shaderFound[s]) {
			REPORT_WARNING("Shader type already loaded, aborting");
			if (buf)
				free(buf);
			return false;
		}
		...
		i32 k = latestVersion;
		for (; k < VER_LAST; k++) {
			if (strlen(desc[i]) + strlen(fnpath) + strlen(filename_prefix[k]) >= (FILENAME_MAX - 1)) {
				REPORT_WARNING("Shader filename too long \"%s%s%s%s\"", fnpath, filename_prefix, desc, filename_suffix[s]);
				if (buf)
					free(buf);
				return false;
			}

			strcpy(fnbuf[s], fnpath);
			strcat(fnbuf[s], filename_prefix[k]);
			strcat(fnbuf[s], desc[i]);

			/* Read source file */
			file = fopen(fnbuf[s], "rb");
			if (file)
				break;
		}

		if (k == VER_LAST || !file) {
			REPORT_WARNING("Couldn't load shader source for \"%s\"", desc);
			if (buf)
				free(buf);
			return false;
		}
		...
		if (!fread(buf, file_len, 1, file)) {
			REPORT_WARNING("Error reading shader source for \"%s\"", desc);
			if (buf)
				free(buf);
			fclose(file);
			return false;
		...
		if (!(shader_id[s] = loadShader(shader_program->getIdentifier(), prefix, buf, shader_type[s]))) {
			u32 j;
			GL_CHECK_ERRORS();
			for (j = 0; j < 5; j++) {
				if (!shaderFound[j])
...
//		char result[SHADER_PROGRAM_ERROR_BUFFER_LENGTH];
	...
	if (!shaderFound[SHADER_TYPE_VERTEX] || !shaderFound[SHADER_TYPE_FRAGMENT])
		REPORT_WARNING("Shader program must have a vertex shader and a fragment shader");

	hdl->sp_id = id;
	hdl->vs_id = shader_id[SHADER_TYPE_VERTEX];
	hdl->fs_id = shader_id[SHADER_TYPE_FRAGMENT];
	if (shaderFound[SHADER_TYPE_GEOMETRY]) hdl->gs_id = shader_id[SHADER_TYPE_GEOMETRY];
	hdl->has_gs = shaderFound[SHADER_TYPE_GEOMETRY];
	if (shaderFound[SHADER_TYPE_CONTROL]) hdl->cs_id = shader_id[SHADER_TYPE_CONTROL];
	hdl->has_cs = shaderFound[SHADER_TYPE_CONTROL];
	if (shaderFound[SHADER_TYPE_EVALUATION]) hdl->es_id = shader_id[SHADER_TYPE_EVALUATION];
	hdl->has_es = shaderFound[SHADER_TYPE_EVALUATION];
	...
	loc = glGetUniformLocation(id, "NormalMatrix");
	GL_CHECK_ERRORS();
	hdl->loc[i++] = loc;
	...
	)
{
	...
	if (hdl) {
		...
		n_shaders -= 2;
		if (hdl->has_gs) {
			glDeleteShader(hdl->gs_id);
			n_shaders--;
		}
		if (hdl->has_cs) {
			glDeleteShader(hdl->cs_id);
			n_shaders--;
		}
		if (hdl->has_es) {
			glDeleteShader(hdl->es_id);
			n_shaders--;
		}
	...
	for (i = 0; i < n_entries; i++) {
		...
		if (entries[i].flags & (UNIFORM_FLAG_TEXTURE | UNIFORM_FLAG_CUBEMAP)) {
			...
			if (entries[i].flags & UNIFORM_FLAG_TEXTURE) {
				...
				thandle = *((GLTextureHandle_t **) entries[i].data);
				texId = thandle->tex_id;
			} else {
				GLCubeMapHandle_t *chandle;
				chandle = *((GLCubeMapHandle_t **) entries[i].data);
			...
			if (j == curr_tex_slot) {
				...
				if (curr_tex_slot >= RC_MAX_TEXTURE_SLOTS || curr_tex_slot >= limits.max_texture_units) {
			...
			switch (entries[i].flags & UNIFORM_FLAG_CLASS_MASK) {
				case (UNIFORM_FLAG_SCALAR): /* Scalar */
					if (entries[i].n == 1)
						glUniform1i(loc, *((i32 *) entries[i].data)); /* Single value */
					else
						glUniform1iv(loc, entries[i].n, (i32 *) entries[i].data); /* Array */
					break;
				case (UNIFORM_FLAG_VECTOR2): /* Vec2 int */
					if (entries[i].n == 1) {
						i32 *v = (i32 *) entries[i].data;
						glUniform2i(loc, v[0], v[1]);
					} else
						glUniform2iv(loc, entries[i].n, (GLint *) entries[i].data);
					break;
				case (UNIFORM_FLAG_VECTOR3):
					if (entries[i].n == 1) {
						...
						glUniform3iv(loc, entries[i].n, (GLint *) entries[i].data);
					break;
				case (UNIFORM_FLAG_VECTOR4):
					if (entries[i].n == 1) {
						...
						glUniform4iv(loc, entries[i].n, (GLint *) entries[i].data);
					break;
				default:
					break;
			}
			...
			switch (entries[i].flags & UNIFORM_FLAG_CLASS_MASK) {
				case (UNIFORM_FLAG_SCALAR): /* Scalar */
					if (entries[i].n == 1)
						glUniform1f(loc, *((f32 *) entries[i].data)); /* Single value */
					else
						glUniform1fv(loc, entries[i].n, (GLfloat *) entries[i].data); /* Array */
					break;
				case (UNIFORM_FLAG_VECTOR2): /* Vec2 float */
					if (entries[i].n == 1) {
						vec2f *v = (vec2f *) entries[i].data;
						glUniform2f(loc, v->x, v->y);
					} else
						glUniform2fv(loc, entries[i].n, (GLfloat *) entries[i].data);
					break;
				case (UNIFORM_FLAG_VECTOR3):
					if (entries[i].n == 1) {
						vec3f *v = (vec3f *) entries[i].data;
						glUniform3f(loc, v->x, v->y, v->z);
					} else
						glUniform3fv(loc, entries[i].n, (GLfloat *) entries[i].data);
					break;
				case (UNIFORM_FLAG_VECTOR4):
					if (entries[i].n == 1) {
						vec4f *v = (vec4f *) entries[i].data;
						glUniform4f(loc, v->x, v->y, v->z, v->w);
					} else
						glUniform4fv(loc, entries[i].n, (GLfloat *) entries[i].data);
					break;
				case (UNIFORM_FLAG_MATRIX2): { /* Mat2 float */
					mat2f mat = *((mat2f *) entries[i].data);
					mat.transpose();
					glUniformMatrix2fv(loc, entries[i].n, GL_FALSE, (GLfloat *) mat.vec);
					} break;
				case (UNIFORM_FLAG_MATRIX3): {
					mat3f mat = *((mat3f *) entries[i].data);
					mat.transpose();
					glUniformMatrix3fv(loc, entries[i].n, GL_FALSE, (GLfloat *) mat.vec);
					} break;
				case (UNIFORM_FLAG_MATRIX4): {
					mat4f mat = *((mat4f *) entries[i].data);
					mat.transpose();
	...
	)
{
	...
	if (n_textures > limits.max_render_targets)
	...
	)
{
	...
	gl_usage = gl_usage_map[usage];
	glBufferDataARB(GL_ARRAY_BUFFER, stride * n, new_array, gl_usage);
	GL_CHECK_ERRORS();
	...
	)
{
	...
	glBufferDataARB(GL_ARRAY_BUFFER, vertex_array->getStride() * n, new_array, gl_usage);
	GL_CHECK_ERRORS();
	...
	)
{
	...
	gl_usage = gl_usage_map[usage];
	glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER, n * type_size, new_array, gl_usage);
	GL_CHECK_ERRORS();
	...
	)
{
	...
	glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER, n * vertex_array->getIndexTypeSize(), new_array, gl_usage);
	GL_CHECK_ERRORS();
