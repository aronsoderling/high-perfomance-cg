...
/*-------------------------------------------------------[ Node getters ]----*/
		 ...
		 * getNodeCount(NodeType) to find out how many nodes that is.
		 *   This method does not allocate any memory. It's up to the caller to
		 * provide an appropriately sized array pointer.
		 *
		 * @param node_list
...
/*--------------------------------------------------[ Resource creators ]----*/
		...
		static VertexArray *createVertexArray(
				...
				AttributeUsage_t usage = USAGE_STATIC
			);
		...
		/**
		 * Create a vertex array for full screen pass
		 *
		 * This creates a Vertex Array to use for full screen passes. The
		 * vertex array will _only_ contain a vec2 attribute "Vertex"
		 *
		 * @param nstride
		 *   The length of each struct
		 *
		 ...
		 *
		 */
		static VertexArray *createFullScreenVertexArray(
				const char		*nidentifier
			);
		 ...
		 * Create a Texture resource from file
		 *
		 * This loads a texture from a file. The file name is also used as the
		 * identifier for the Texture object. The filter is set to trilinear,
		 * and the wrap to repeat by default. Also, mipmaps are generated.
		 *
		 * @param filename
		 *   The filename of the image (preferably .png or .bmp)
		 *
...
/*---------------------------------------------------[ Resource getters ]----*/
		...
		/**
		 * Get list of Resources, C++ style
		 *
		 */
		template<class T> // MTODO: T
...
/*===========================================================================*/
	...
	private:
		template<class T>
		static const char *prepNewNodeName(const char *nname);
		template<class T>
		static T *ensureNodeSanity(T *node);
		template<class T>
		static T *createEmptyNode(const char *nname);

		template<class T>
		static const char *prepNewResourceName(const char *nname);
		template<class T>
		static T *ensureResourceSanity(T *resource);
		...
		template<class T>
		static ResourceList *getInternalResourceList();
